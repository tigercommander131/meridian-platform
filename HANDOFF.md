...
**Last updated:** 2026-06-21 — **Adaptable columns + 2000-course LMS org.** New `lms` org from the LMS xlsx (full source row per course in `attributes` JSONB); board + course cards have a **column picker** to add/remove any variable. Course filters (search · type · location · status · time-window). AI report card reworked. M2 + airline-bold v2 + Settings live. M1 live. (Local Postgres still down → verified via prod build + engine tests; DB seed runs on Railway.)
